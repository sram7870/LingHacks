...
class RelationalAnalyzer:
    ...
    def _get_total_papers_count(self) -> int:
        ...
        if self.graph_client is None or not getattr(self.graph_client, "available", False):
            ...
            # Return a minimum corpus size to allow RPA to proceed with fallback scoring
            return max(2, len(self.landscape_paper_titles) if self.landscape_paper_titles else 2)
    ...
    def _compute_cas(self, extracted_claims: List[dict], input_title: str, K: int = 20) -> dict:
        ...
        if self.graph_client is None or not getattr(self.graph_client, "available", False):
            logger.warning("Graph unavailable; returning fallback CAS scores")
            per_claim_cas = []
            all_cas_scores = []
            
            for claim in extracted_claims:
                # Return moderate consensus (0.5) for all claims as fallback
            ...
            return {
                ...
                "aggregate_cas": 0.5,
                "cas_interpretation": "Moderate Consensus (Graph Unavailable)"
            }
        ...
        try:
            ...
            logger.warning("Query failed, using fallback CAS: %s", exc)
            ...
            return {
                "per_claim_cas": per_claim_cas,
                "aggregate_cas": 0.5,
                "cas_interpretation": "Moderate Consensus (Query Failed)"
            }
    ...
    def _compute_fci(self, paper_id: str, paper_title: str, stance_label: str) -> dict:
        ...
        # Fallback when graph is unavailable
        if self.graph_client is None or not getattr(self.graph_client, "available", False):
            logger.warning("Graph unavailable; returning fallback FCI scores")
            return {
                "fci_score": 50,
                "fci_label": "Moderate Controversy",
                "subgraph_paper_count": len(self.landscape_paper_titles) if self.landscape_paper_titles else 1,
                "edge_controversy_ratio": 0.5,
                "stance_distribution": {}
            }
        ...
        try:
            ...
            logger.warning("FCI computation failed, using fallback: %s", exc)
            return {
                "fci_score": 50,
                "fci_label": "Moderate Controversy (Query Failed)",
                "subgraph_paper_count": len(self.landscape_paper_titles) if self.landscape_paper_titles else 1,
                "edge_controversy_ratio": 0.5,
                "stance_distribution": {"supporting": 0, "opposing": 0, "neutral": 0}
            }
    ...
    def _compute_mss(
        ...
        Percentile ranking within connected papers by methodology quality.
        """
        # Fallback when graph is unavailable
        if self.graph_client is None or not getattr(self.graph_client, "available", False):
            logger.warning("Graph unavailable; returning fallback MSS scores")
            return {
                "mss_percentile": 50.0,
                "mss_label": "Average (Graph Unavailable)",
        ...
        try:
            with self.graph_client.driver.session() as session:
                # Query papers connected by CONTRADICTS or SUPPORTS relations
                query_connected = """
                    MATCH (input:Paper {title: $title})-[r:CONTRADICTS|SUPPORTS]-(other:Paper)
                    WHERE other.title <> $title
                """
                if self.landscape_paper_titles:
                    query_connected += " AND other.title IN $titles"
                
                query_connected += """
                    RETURN DISTINCT other.title AS title,
                                    COALESCE(other.methodology_quality, other.methodological_quality, 0.5) AS methodology_quality,
                                    other.stance_label AS stance_label,
                                    type(r) AS rel_type
                """
                
                res_connected = session.run(query_connected, title=paper_title, titles=self.landscape_paper_titles)
                connected_papers = {}
                paper_max_sims = {}
                
                for record in res_connected:
                    title = record["title"]
                ...
                # Query similar papers by embedding
                if extracted_claims:
                    query_similar = """
                        MATCH (input:Paper {title: $title})-[:SUPPORTED_BY|MAKES_CLAIM]-(c_input:Claim)
                        WHERE c_input.embedding IS NOT NULL
                        WITH DISTINCT c_input.embedding AS input_emb
                        MATCH (other:Paper)-[:SUPPORTED_BY|MAKES_CLAIM]-(c_other:Claim)
                        WHERE other.title <> $title AND c_other.embedding IS NOT NULL
                    """
                    if self.landscape_paper_titles:
                        query_similar += " AND other.title IN $titles"
                    
                    query_similar += """
                        RETURN DISTINCT other.title AS title,
                                        COALESCE(other.methodology_quality, other.methodological_quality, 0.5) AS methodology_quality,
                                        other.stance_label AS stance_label,
                                        MAX(gds.similarity.cosine(input_emb, c_other.embedding)) AS max_sim
                        ORDER BY max_sim DESC
                    """
                    
                    res_similar = session.run(query_similar, title=paper_title, titles=self.landscape_paper_titles)
                    for record in res_similar:
                        title = record["title"]
                        if title not in paper_max_sims:
                            paper_max_sims[title] = {
                                "title": title,
                                "max_sim": record["max_sim"],
                                "methodology_quality": record["methodology_quality"] if record["methodology_quality"] is not None else 0.5,
                                "stance_label": record["stance_label"] or "neutral"
                            }
            ...
            logger.warning("MSS computation failed, using fallback: %s", exc)
            return {
                "mss_percentile": 50.0,
                "mss_label": "Average (Query Failed)",
                "contradicting_papers_median_quality": 0.0,
                "methodological_underdog": False,
                "comparison_pool_size": 0
            }
    ...
    def _compute_cns(
        ...
        # Fallback when graph is unavailable
        if self.graph_client is None or not getattr(self.graph_client, "available", False):
            logger.warning("Graph unavailable; returning fallback CNS scores")
            per_claim_cns = []
            all_cns_scores = []
            
            for claim in extracted_claims:
                # Return moderate novelty (0.5) for all claims as fallback
                per_claim_cns.append(ClaimCNS(
                    claim_text=claim.get("text", ""),
                    cns_score=0.5,
                    most_similar_existing_claim="Unknown",
                    similarity_score=0.5,
                    replication_candidate=False
                ))
                all_cns_scores.append(0.5)
            
            return {
                "per_claim_cns": per_claim_cns,
                "aggregate_cns": 0.5,
                "cns_interpretation": "Moderate Novelty (Graph Unavailable)"
            }
        ...
        try:
            ...
            logger.warning("CNS query failed, using fallback: %s", exc)
            per_claim_cns = []
            all_cns_scores = []
            ...
            return {
                "per_claim_cns": per_claim_cns,
                "aggregate_cns": 0.5,
                "cns_interpretation": "Moderate Novelty (Query Failed)"
            }
    ...
    def _compute_tfp(self, paper_id: str, paper_title: str, paper_year: int, stance_label: str) -> dict:
        ...
        # Fallback when graph is unavailable
        if self.graph_client is None or not getattr(self.graph_client, "available", False):
            logger.warning("Graph unavailable; returning fallback TFP scores")
            return {
                "field_trajectory_at_publication": "Insufficient Data",
                "alignment_with_trajectory": "Neutral",
                "debate_maturity": "Mid-Debate",
                "papers_published_same_period": 0,
                "drift_velocity_at_publication": 0.0
            }
        ...
        try:
            ...
            logger.warning("TFP computation failed, using fallback: %s", exc)
            return {
                "field_trajectory_at_publication": "Insufficient Data",
                "alignment_with_trajectory": "Neutral",
                "debate_maturity": "Mid-Debate",
